...
function MultiselectDropdown(options) {
  ...
  for (let index = 0; index < obj.length; index++) {
    console.log(JSON.stringify(obj[index].market));
  }
  options = obj;
  ...
  document.querySelectorAll("select[multiple]").forEach((el, k) => {
    ...
    el.loadOptions = () => {
      ...
      Array.from(el.options).map((o) => {
        ...
        op.appendChild(ic);
        op.appendChild(newEl("label", { text: o.options }));
...
//     document.getElementsByClassName("multiselect-dropdown-list").innerHTML = obj[index].market;
//   }
//   for (let index = 0; index < obj.length; index++) {
